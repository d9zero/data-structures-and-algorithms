...
const createServer = () => {
  ...
  const app = express();
  app.get('hello', (req,res) => res.status(200).send('Pancakes are delicious'));
  app.get('/aboutme', (req,res) => res.status(200).send ('A Toyota Tacoma is not a full size truck'));
  app.get('/favoritefoods', (req,res) => {
    const faveFood = ['Warm, decadent Cinnabons', 'Medium Rare Beef/Pork Sirloin', 'Uninterruptable Caffeine Supply', 'Compliments, like vitamins'];
    res.status(200).send(faveFood);
  });
  app.use("*", (req,res) => {
    res.status(404).send('wrong turn buddy');
  });
...
const totalSum = (input) => {
  // Solution code here...
  let totalCount = input.reduce((acc, val) => acc + val.reduce((acc2, val2) => acc2 + val2, 0), 0);
  return totalCount;
